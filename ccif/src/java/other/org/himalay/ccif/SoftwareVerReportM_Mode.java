...
public  class SoftwareVerReportM_Mode extends BinStruct implements PublicBinMsg{
    ...
    private void init()
    {
        ...
        applicationsInReport= new ArrayList<softwareVerMMsg>();applicationsInReport.setMemberSize(0);
    }
    ...
    preRead();
        ...
        for (; istream.available() > 0 ; ){     softwareVerMMsg temp;    temp = new softwareVerMMsg();    retVal += temp.read(istream);    applicationsInReport.add(temp);}
    ...
    preWrite();
        ...
        {ArrayList<softwareVerMMsg> temp1 = applicationsInReport;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     softwareVerMMsg temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("SoftwareVerReportM_Mode\n");
    dc.increaseIndent();
        int retVal= 0;
        // write applicationsInReport
        { ArrayList<softwareVerMMsg> temp1 = applicationsInReport;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     softwareVerMMsg element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
