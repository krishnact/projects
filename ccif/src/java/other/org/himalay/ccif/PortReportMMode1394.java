...
public  class PortReportMMode1394 extends BinStruct implements PublicBinMsg{
    ...
    private void init()
    {
        ...
        numberOfConnectedDevices= new ByteArray();numberOfConnectedDevices.setSizeType("FIRST_UI8");
    }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("PortReportMMode1394\n");
    dc.increaseIndent();
        int retVal= 0;
        // write totalNumberOfNodes
        dc.indent();dc.getPs().println("totalNumberOfNodes="+totalNumberOfNodes+"(0x"+ Integer.toHexString(totalNumberOfNodes) + ")" );
        // write bf1
        {dc.indent();dc.getPs().print("reserved: ");dc.getPs().println(BitField_8.toDisplayString(getReserved(),2));dc.indent();dc.getPs().print("loopStatus: ");dc.getPs().println(BitField_8.toDisplayString(getLoopStatus(),1));dc.indent();dc.getPs().print("rootStatus: ");dc.getPs().println(BitField_8.toDisplayString(getRootStatus(),1));dc.indent();dc.getPs().print("cycleMasterStatus: ");dc.getPs().println(BitField_8.toDisplayString(getCycleMasterStatus(),1));dc.indent();dc.getPs().print("host_ad_sourceSelectionStatus: ");dc.getPs().println(BitField_8.toDisplayString(getHost_ad_sourceSelectionStatus(),1));dc.indent();dc.getPs().print("port_1ConnectionStatus: ");dc.getPs().println(BitField_8.toDisplayString(getPort_1ConnectionStatus(),1));dc.indent();dc.getPs().print("port_2ConnectionStatus: ");dc.getPs().println(BitField_8.toDisplayString(getPort_2ConnectionStatus(),1));}
        // write numberOfConnectedDevices
        dc.indent();dc.getPs().print("numberOfConnectedDevices: "+numberOfConnectedDevices.getSize()+"(0x"+Integer.toHexString(numberOfConnectedDevices.getSize())+")\n");this.numberOfConnectedDevices.dump(dc);
dc.decreaseIndent();
        return retVal;
    }
