...
public  class shortform_virtual_channel_table_section_ extends   SCTE65TableFactory.SCTE65Table { //Concrete type is shortform_virtual_channel_table_section
  ...
  preRead();
    int retVal= 0;
        ...
        // read VCT_ID
        {VCT_ID=istream.readUnsignedShort(); retVal+=2;}
        // read subTable
        {IntegerHolder iHolder= new IntegerHolder();DataInputStream disTemp = istream;subTable=S_VCT_SubtableFactory.createMsg(getTable_subtype(),disTemp,iHolder); retVal+= iHolder.getValue();        }
        // read CRC
...
postRead();
    ...
    preWrite();
        ...
        // write VCT_ID
        ostream.writeShort(VCT_ID); retVal +=2;
        // write subTable
        if (subTable!=null)retVal +=subTable.write(ostream);
        // write CRC
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("shortform_virtual_channel_table_section_\n");
    dc.increaseIndent();
        int retVal= 0;
        // write tableHeader
        ...
        // write VCT_ID
        dc.indent();dc.getPs().println("VCT_ID="+VCT_ID+"(0x"+ Integer.toHexString(VCT_ID) + ")" );
        // write subTable
        if ( subTable != null ) {dc.indent();dc.getPs().println("subTable") ;retVal +=subTable.dump(dc);}
        // write CRC
