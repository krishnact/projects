...
public class transport_protocol_descriptor
    ...
    public int readNoHeader(DataInputStream istream) throws IOException {
        ...
        if (sizeLimit > 0) {
            ...
            istream = new DataInputStream(baTemp);
            {
                // read id
                {
                    id = istream.readUnsignedShort();
                    retVal += 2;
                }
                // read label
                {
                    label = (short) (istream.readUnsignedByte());
                    retVal += 1;
                }
                // read url
                {
    ...
    public int dump(DumpContext dc) throws IOException {
        ...
        // write id
        dc.indent();
        dc.getPs().println("id=" + id + "(0x" + Integer.toHexString(id) + ")");
        // write label
        dc.indent();
        dc.getPs().println(
            "label=" + label + "(0x" + Integer.toHexString(label) + ")");
        // write url
        dc.indent();
        dc.getPs().print(
            "url: " + url.getSize() + "(0x"
            + Integer.toHexString(url.getSize()) + ")\n");
        this.url.dump(dc);
        dc.decreaseIndent();
        return retVal;
    }
