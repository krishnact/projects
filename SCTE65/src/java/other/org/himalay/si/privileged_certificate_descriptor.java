...
public  class privileged_certificate_descriptor extends   app_desc_baseFactory.app_desc_base { //Concrete type is privileged_certificate_descriptor


// member static classes
    public  static  class certificate_identifier_byte extends BinStruct implements PublicBinMsg{
...
privileged_certificate_descriptor parent;
    public certificate_identifier_byte (privileged_certificate_descriptor parent) // throws Exception
    {
        this.parent= parent;
            init();
    }
    ...
    {
        // Initialize data
        data= new ByteArray();data.setSizeType("FIXED");data.setSize(20);
    }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("certificate_identifier_byte\n");
    dc.increaseIndent();
        int retVal= 0;
        // write data
...
// End of code
    ...
    private void init()
    {
        ...
        certificates= new ArrayList<certificate_identifier_byte>();certificates.setMemberSize(0);
    }
  ...
  preRead();
    int retVal= 0;
                // read certificates
        { int ilimit= retVal + (desc_header.length + (0)); for (; retVal < ilimit;){     certificate_identifier_byte temp;    temp = new certificate_identifier_byte(this);    retVal += temp.read(istream);    certificates.add(temp);}}
...
postRead();
    ...
    preWrite();
        ...
        { /** fix dependent sizes for certificates**/ desc_header.length=((short)certificates.getSize()); }
    
                // write desc_header
        if (desc_header!=null)retVal +=desc_header.write(ostream);
        // write certificates
        {ArrayList<certificate_identifier_byte> temp1 = certificates;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     certificate_identifier_byte temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("privileged_certificate_descriptor\n");
    dc.increaseIndent();
        int retVal= 0;
        // write desc_header
        if ( desc_header != null ) {dc.indent();dc.getPs().println("desc_header") ;retVal +=desc_header.dump(dc);}
        // write certificates
        { ArrayList<certificate_identifier_byte> temp1 = certificates;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     certificate_identifier_byte element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            certificates.add(val); return certificates.size();
    }
    
    
    public int removeFromCertificates(certificate_identifier_byte val)
    {
            certificates.remove(val); return certificates.size();
    }
    
    
    public int removeNthFromCertificates(int idx)
    {
            certificates.remove(idx); return certificates.size();
    }
    
    
    public int emptyCertificates(int idx)
    {
            certificates.clear(); return certificates.size();
    }
