...
public class xait_section extends SCTE65TableFactory.SCTE65Table { // Concrete
    // type is
    // xait_section
    ...
    public static class app_data extends BinStruct implements PublicBinMsg {
        ...
        private void init() {
            ...
            app_descriptors = new ArrayList<app_desc_baseFactory.app_desc_base>();
        ...
        public int read(DataInputStream istream) throws IOException {
            ...
            // read org_id
            {
                org_id = (long) (BinPrimitive.readUI32(istream));
                retVal += 4;
            }
            // read app_id
            {
                app_id = istream.readUnsignedShort();
                retVal += 2;
            }
            // read app_control_code
            {
                app_control_code = (short) (istream.readUnsignedByte());
                retVal += 1;
            }
            // read bf
            ...
            {
                int ilimit = retVal
                             + (getApplication_descriptors_loop_length() + (0));
                for (; retVal < ilimit;) {
                    app_desc_baseFactory.app_desc_base temp; /*
															 * Generic classes
															 * are abstract, so
															 * we can not invoke
															 * new
        ...
        public int write(DataOutputStream ostream) throws IOException {
            ...
            {
                /** fix dependent sizes for app_descriptors **/
                setApplication_descriptors_loop_length((short) app_descriptors
                                                       .getSize());
            }

            // write org_id
            BinPrimitive.writeUI32(ostream, org_id);
            retVal += 4;
            // write app_id
            ostream.writeShort(app_id);
            retVal += 2;
            // write app_control_code
            ostream.writeByte(app_control_code);
            retVal += 1;
            // write bf
            ...
            {
                ArrayList<app_desc_baseFactory.app_desc_base> temp1 = app_descriptors;
                for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                    app_desc_baseFactory.app_desc_base temp2 = temp1.get(iIdx);
                    if (temp2 != null)
        ...
        public int dump(DumpContext dc) throws IOException {
            ...
            .println(
                "org_id=" + org_id + "(0x"
                + Long.toHexString(org_id) + ")");
            // write app_id
            dc.indent();
            dc.getPs().println(
                "app_id=" + app_id + "(0x" + Integer.toHexString(app_id)
                + ")");
            // write app_control_code
            dc.indent();
            dc.getPs().println(
                ...
                + app_control_code
                + "(0x"
                + Integer.toHexString(app_control_code)
                + ")"
                + " ("
                + Table_AppControlCode
                .getMnemonic(app_control_code) + ")");
            // write bf
            {
                dc.indent();
                dc.getPs().print("reserved: ");
                dc.getPs()
                .println(BitField_8.toDisplayString(getReserved(), 4));
                dc.indent();
                dc.getPs().print("application_descriptors_loop_length: ");
                dc.getPs().println(
                    BitField_8.toDisplayString(
                        getApplication_descriptors_loop_length(), 12));
            }
            // write app_descriptors
            {
                ArrayList<app_desc_baseFactory.app_desc_base> temp1 = app_descriptors;
                for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                    app_desc_baseFactory.app_desc_base element = temp1
                            .get(iIdx);
                    dc.indent();
        ...
        // public ArrayList<app_desc_baseFactory.app_desc_base>
        // getApp_descriptors()
        // {
        ...
        // public void
        // setApp_descriptors(ArrayList<app_desc_baseFactory.app_desc_base> val)
        // {
        ...
        public void setReserved(int val) {
            bf.setValue((bf.getValue() & 0xffff0fff)
                        | ((val << 12) & 0x0000f000));
        }
        ...
        public void setApplication_descriptors_loop_length(int val) {
            bf.setValue((bf.getValue() & 0xfffff000)
                        | ((val << 0) & 0x00000fff));
        }

        public int addToApp_descriptors(app_desc_baseFactory.app_desc_base val) {
            app_descriptors.add(val);
            return app_descriptors.size();
        }

        public int removeFromApp_descriptors(
            app_desc_baseFactory.app_desc_base val) {
            app_descriptors.remove(val);
            return app_descriptors.size();
        }

        public int removeNthFromApp_descriptors(int idx) {
            app_descriptors.remove(idx);
            return app_descriptors.size();
        }

        public int emptyApp_descriptors(int idx) {
            app_descriptors.clear();
            return app_descriptors.size();
        }
    ...
    private void init() {
        ...
        common_descriptors = new ArrayList<app_desc_baseFactory.app_desc_base>();
        common_descriptors.setMemberSize(0);
        // Initialize bf5
        bf5 = new BitField_16();
        // Initialize appDatum
        appDatum = new ArrayList<app_data>();
    ...
    public int write(DataOutputStream ostream) throws IOException {
        ...
        {
            /** fix dependent sizes for common_descriptors **/
            setCommon_descriptors_length((short) common_descriptors.getSize());
        }

        {
            /** fix dependent sizes for appDatum **/
            setApplication_loop_length((short) appDatum.getSize());
        }
        ...
        {
            ArrayList<app_desc_baseFactory.app_desc_base> temp1 = common_descriptors;
            for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                app_desc_baseFactory.app_desc_base temp2 = temp1.get(iIdx);
                if (temp2 != null)
                    retVal += temp2.write(ostream);
        ...
        // write appDatum
        {
            ArrayList<app_data> temp1 = appDatum;
            for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                app_data temp2 = temp1.get(iIdx);
                if (temp2 != null)
                    retVal += temp2.write(ostream);
            }
        }
    ...
    public int dump(DumpContext dc) throws IOException {
        ...
        // write bf2
        {
            ...
            dc.getPs().println(
                BitField_8.toDisplayString(getTest_application_flag(), 1));
            dc.indent();
            dc.getPs().print("application_type: ");
            dc.getPs().println(
                BitField_8.toDisplayString(getApplication_type(), 15));
        }
        // write bf3
        {
            dc.indent();
        ...
        // write bf4
        {
            ...
            dc.getPs().println(BitField_8.toDisplayString(getReserved3(), 4));
            dc.indent();
            dc.getPs().print("common_descriptors_length: ");
            dc.getPs().println(
                BitField_8.toDisplayString(getCommon_descriptors_length(),
                                           12));
        }
        // write common_descriptors
        {
            ArrayList<app_desc_baseFactory.app_desc_base> temp1 = common_descriptors;
            for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                app_desc_baseFactory.app_desc_base element = temp1.get(iIdx);
                dc.indent();
                dc.getPs().println(iIdx);
                if (element != null) {
                    dc.indent();
        ...
        // write bf5
        {
            ...
            dc.getPs().println(BitField_8.toDisplayString(getReserved4(), 4));
            dc.indent();
            dc.getPs().print("application_loop_length: ");
            dc.getPs().println(
                BitField_8
                .toDisplayString(getApplication_loop_length(), 12));
        }
        // write appDatum
        {
            ArrayList<app_data> temp1 = appDatum;
            for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
                app_data element = temp1.get(iIdx);
                dc.indent();
                dc.getPs().println(iIdx);
                if (element != null) {
                    dc.indent();
    ...
    // public void
    // setCommon_descriptors(ArrayList<app_desc_baseFactory.app_desc_base> val)
    // {
    ...
    public int addToCommon_descriptors(app_desc_baseFactory.app_desc_base val) {
        common_descriptors.add(val);
        return common_descriptors.size();
    }

    public int removeFromCommon_descriptors(
        app_desc_baseFactory.app_desc_base val) {
        common_descriptors.remove(val);
        return common_descriptors.size();
    }

    public int removeNthFromCommon_descriptors(int idx) {
        common_descriptors.remove(idx);
        return common_descriptors.size();
    }
