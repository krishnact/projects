...
public  class application_storage_descriptor extends   app_desc_baseFactory.app_desc_base { //Concrete type is application_storage_descriptor
  ...
  preRead();
    ...
    istream = new DataInputStream(baTemp);
    {
        ...
        {storage_priority=istream.readUnsignedShort(); retVal+=2;}
        // read launch_order
        {launch_order=(short)(istream.readUnsignedByte()); retVal+=1;}
...
postRead();
    ...
    preWrite();
        ...
        ostream.writeShort(storage_priority); retVal +=2;
        // write launch_order
        ostream.writeByte(launch_order); retVal +=1;
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("application_storage_descriptor\n");
    dc.increaseIndent();
        int retVal= 0;
        // write desc_header
        if ( desc_header != null ) {dc.indent();dc.getPs().println("desc_header") ;retVal +=desc_header.dump(dc);}
        // write storage_priority
        dc.indent();dc.getPs().println("storage_priority="+storage_priority+"(0x"+ Integer.toHexString(storage_priority) + ")" );
        // write launch_order
        dc.indent();dc.getPs().println("launch_order="+launch_order+"(0x"+ Integer.toHexString(launch_order) + ")" );
dc.decreaseIndent();
        return retVal;
    }
