...
public  class system_time_table_section_ extends   SCTE65TableFactory.SCTE65Table { //Concrete type is system_time_table_section
...
system_time_table_section_ parent;
    ...
    preRead();
        int retVal= 0;
                     ...
                     // read zero
        {zero=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read systemTime
        {systemTime=(long)(BinPrimitive.readUI32(istream) ); retVal+=4;}
        // read GPS_UTC_Offset
        {GPS_UTC_Offset=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read descriptor
    ...
    preWrite();
        ...
        { /** fix dependent sizes for descriptor **/  }
                ...
                // write zero
        ostream.writeByte(zero); retVal +=1;
        // write systemTime
        BinPrimitive.writeUI32(ostream,systemTime); retVal +=4;
        // write GPS_UTC_Offset
        ostream.writeByte(GPS_UTC_Offset); retVal +=1;
        // write descriptor
...
postWrite();
    ...
    dc.increaseIndent();
        int retVal= 0;
        // write zero
        dc.indent();dc.getPs().println("zero="+zero+"(0x"+ Integer.toHexString(zero) + ")" );
        // write systemTime
        dc.indent();dc.getPs().println("systemTime="+systemTime+"(0x"+ Long.toHexString(systemTime)+")") ;
        // write GPS_UTC_Offset
        dc.indent();dc.getPs().println("GPS_UTC_Offset="+GPS_UTC_Offset+"(0x"+ Integer.toHexString(GPS_UTC_Offset) + ")" );
        // write descriptor
...
postWrite();
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("system_time_table_section_\n");
    dc.increaseIndent();
        int retVal= 0;
        // write tableHeader
