...
public  class HostPropertiesReq extends   APDUBaseFactory.APDUBase { //Concrete type is HostPropertiesReq
    ...
    private void init()
    {
        ...
        // Initialize hpr
        hpr= new ArrayList<HostPropertiesReqMsg>();hpr.setMemberSize(0);
    }
  ...
  preRead();
    int retVal= 0;
        ...
        // read hpr
        for (; istream.available() > 0 ; ){     HostPropertiesReqMsg temp;    temp = new HostPropertiesReqMsg();    retVal += temp.read(istream);    hpr.add(temp);}
...
postRead();
    ...
    preWrite();
        ...
        // write hpr
        {ArrayList<HostPropertiesReqMsg> temp1 = hpr;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     HostPropertiesReqMsg temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("HostPropertiesReq\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        ...
        // write hpr
        { ArrayList<HostPropertiesReqMsg> temp1 = hpr;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     HostPropertiesReqMsg element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
