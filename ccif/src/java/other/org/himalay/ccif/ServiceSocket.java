...
public  class ServiceSocket extends BinStruct implements PublicBinMsg {
    ...
    public int read(DataInputStream istream) throws IOException
    {
        ...
        // read protocolFlag
        {
            protocolFlag=(short)(istream.readUnsignedByte());
            retVal+=1;
        }
        // read localPort
        {
            localPort=istream.readUnsignedShort();
            retVal+=2;
        }
        // read remotePort
        {
            remotePort=istream.readUnsignedShort();
            retVal+=2;
        }
        // read remoteAddressType
        {
            remoteAddressType=(short)(istream.readUnsignedByte());
            retVal+=1;
        }
        // read name
        if (remoteAddressType==0x00) {
            name = new ByteArrayWrapper_ui8();
    ...
    public int write(DataOutputStream ostream) throws IOException
    {
        ...
        // write protocolFlag
        ostream.writeByte(protocolFlag);
        retVal +=1;
        // write localPort
        ostream.writeShort(localPort);
        retVal +=2;
        // write remotePort
        ostream.writeShort(remotePort);
        retVal +=2;
        // write remoteAddressType
        ostream.writeByte(remoteAddressType);
        retVal +=1;
        // write name
    ...
    public int dump(DumpContext dc) throws IOException
    {
        ...
        // write protocolFlag
        dc.indent();
        dc.getPs().println("protocolFlag="+protocolFlag+"(0x"+ Integer.toHexString(protocolFlag) + ")" );
        // write localPort
        dc.indent();
        dc.getPs().println("localPort="+localPort+"(0x"+ Integer.toHexString(localPort) + ")" );
        // write remotePort
        dc.indent();
        dc.getPs().println("remotePort="+remotePort+"(0x"+ Integer.toHexString(remotePort) + ")" );
        // write remoteAddressType
        dc.indent();
        dc.getPs().println("remoteAddressType="+remoteAddressType+"(0x"+ Integer.toHexString(remoteAddressType) + ")" );
        // write name
        if ( name != null ) {
            dc.indent();
