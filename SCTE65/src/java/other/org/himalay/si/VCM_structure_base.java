...
public  class VCM_structure_base extends   S_VCT_SubtableFactory.S_VCT_Subtable { //Concrete type is VCM_structure


// member static classes
    public  static  class virtual_channel extends BinStruct implements PublicBinMsg{
...
virtual_channel parent;
    ...
    preRead();
        int retVal= 0;
                     ...
                     // read CDS_reference
        {CDS_reference=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read program_number
        {program_number=istream.readUnsignedShort(); retVal+=2;}
        // read MMS_reference
        {MMS_reference=(short)(istream.readUnsignedByte()); retVal+=1;}
    ...
    preWrite();
        int retVal= 0;
                ...
                // write CDS_reference
        ostream.writeByte(CDS_reference); retVal +=1;
        // write program_number
        ostream.writeShort(program_number); retVal +=2;
        // write MMS_reference
        ostream.writeByte(MMS_reference); retVal +=1;
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("MpegInfo\n");
    dc.increaseIndent();
        int retVal= 0;
        // write CDS_reference
        dc.indent();dc.getPs().println("CDS_reference="+CDS_reference+"(0x"+ Integer.toHexString(CDS_reference) + ")" );
        // write program_number
        dc.indent();dc.getPs().println("program_number="+program_number+"(0x"+ Integer.toHexString(program_number) + ")" );
        // write MMS_reference
        dc.indent();dc.getPs().println("MMS_reference="+MMS_reference+"(0x"+ Integer.toHexString(MMS_reference) + ")" );
dc.decreaseIndent();
        return retVal;
    }
...
// End of code
// member static classes
    public  static  class Non_MpegInfo extends BinStruct implements PublicBinMsg{
...
virtual_channel parent;
    ...
    preRead();
        ...
        // read reserver_zero
        {reserver_zero=istream.readUnsignedShort(); retVal+=2;}
    ...
    preWrite();
        ...
        ostream.writeByte(CDS_reference); retVal +=1;
        // write bf41
        ostream.writeByte(bf41.getValue()); retVal +=1;
        // write reserver_zero
        ostream.writeShort(reserver_zero); retVal +=2;
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("Non_MpegInfo\n");
    dc.increaseIndent();
        int retVal= 0;
        // write CDS_reference
        dc.indent();dc.getPs().println("CDS_reference="+CDS_reference+"(0x"+ Integer.toHexString(CDS_reference) + ")" );
        // write bf41
        {dc.indent();dc.getPs().print("scrambled: ");dc.getPs().println(BitField_8.toDisplayString(getScrambled(),1));dc.indent();dc.getPs().print("video_standard: ");dc.getPs().println(BitField_8.toDisplayString(getVideo_standard(),4));}
        // write reserver_zero
        dc.indent();dc.getPs().println("reserver_zero="+reserver_zero+"(0x"+ Integer.toHexString(reserver_zero) + ")" );
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
        bf41.setValue ( (bf41.getValue() & 0xffffff7f) | ( (val << 7 ) & 0x00000080));
    }
    ...
    {
        bf41.setValue ( (bf41.getValue() & 0xfffffff0) | ( (val << 0 ) & 0x0000000f));
    }
...
VCM_structure_base parent;
    ...
    preRead();
        ...
        // read src_or_app_id
        {src_or_app_id=istream.readUnsignedShort(); retVal+=2;}
        // read MpegInfoInstance
        if ( getTransport_type() == 0 ){ MpegInfoInstance = new MpegInfo(this);retVal += MpegInfoInstance.read(istream);        }
        // read Non_MpegInfoInstance
        if ( getTransport_type() != 0 ){ Non_MpegInfoInstance = new Non_MpegInfo(this);retVal += Non_MpegInfoInstance.read(istream);        }
        // read descriptors
        if (parent.getDescriptorsIncluded() ==1 ){ descriptors = new DescriptorCollection();retVal += descriptors.read(istream);        }
    ...
    preWrite();
        ...
        // write src_or_app_id
        ostream.writeShort(src_or_app_id); retVal +=2;
        // write MpegInfoInstance
        if (MpegInfoInstance!=null)retVal +=MpegInfoInstance.write(ostream);
        // write Non_MpegInfoInstance
        if (Non_MpegInfoInstance!=null)retVal +=Non_MpegInfoInstance.write(ostream);
        // write descriptors
...
postWrite();
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("virtual_channel\n");
    dc.increaseIndent();
        int retVal= 0;
        // write bf1
        {dc.indent();dc.getPs().print("virtual_channel_number: ");dc.getPs().println(BitField_8.toDisplayString(getVirtual_channel_number(),12));}
        // write bf2
        {dc.indent();dc.getPs().print("application_virtual_channel: ");dc.getPs().println(BitField_8.toDisplayString(getApplication_virtual_channel(),1));dc.indent();dc.getPs().print("path_select: ");dc.getPs().println(BitField_8.toDisplayString(getPath_select(),1));dc.indent();dc.getPs().print("transport_type: ");dc.getPs().println(BitField_8.toDisplayString(getTransport_type(),1));dc.indent();dc.getPs().print("channel_type: ");dc.getPs().println(BitField_8.toDisplayString(getChannel_type(),4));}
        // write src_or_app_id
        dc.indent();dc.getPs().println("src_or_app_id="+src_or_app_id+"(0x"+ Integer.toHexString(src_or_app_id) + ")" );
        // write MpegInfoInstance
        if ( MpegInfoInstance != null ) {dc.indent();dc.getPs().println("MpegInfoInstance") ;retVal +=MpegInfoInstance.dump(dc);}
        // write Non_MpegInfoInstance
        if ( Non_MpegInfoInstance != null ) {dc.indent();dc.getPs().println("Non_MpegInfoInstance") ;retVal +=Non_MpegInfoInstance.dump(dc);}
        // write descriptors
...
// End of code
    ...
    private void init()
    {
        ...
        vcRecords= new ArrayList<virtual_channel>();vcRecords.setMemberSize(0);
    }
    ...
    preRead();
        ...
        retVal += bf2.read(istream);
        // read activationTime
        {activationTime=(long)(BinPrimitive.readUI32(istream) ); retVal+=4;}
        // read numberOfVCRecords
        {numberOfVCRecords=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read vcRecords
        for (int iIdx=0; iIdx <numberOfVCRecords + (0); iIdx++){     virtual_channel temp;    temp = new virtual_channel(this);    retVal += temp.read(istream);    vcRecords.add(temp);}
    ...
    preWrite();
        ...
        { /** fix dependent sizes for vcRecords**/ numberOfVCRecords=((short)vcRecords.getCount()); }
        ...
        BinPrimitive.writeUI32(ostream,activationTime); retVal +=4;
        // write numberOfVCRecords
        ostream.writeByte(numberOfVCRecords); retVal +=1;
        // write vcRecords
        {ArrayList<virtual_channel> temp1 = vcRecords;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     virtual_channel temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("VCM_structure_base\n");
    dc.increaseIndent();
        int retVal= 0;
        // write bf1
        {dc.indent();dc.getPs().print("zero1: ");dc.getPs().println(BitField_8.toDisplayString(getZero1(),2));dc.indent();dc.getPs().print("descriptorsIncluded: ");dc.getPs().println(BitField_8.toDisplayString(getDescriptorsIncluded(),1));dc.indent();dc.getPs().print("zero2: ");dc.getPs().println(BitField_8.toDisplayString(getZero2(),5));}
        // write bf2
        {dc.indent();dc.getPs().print("splice: ");dc.getPs().println(BitField_8.toDisplayString(getSplice(),1));dc.indent();dc.getPs().print("zero3: ");dc.getPs().println(BitField_8.toDisplayString(getZero3(),7));}
        // write activationTime
        dc.indent();dc.getPs().println("activationTime="+activationTime+"(0x"+ Long.toHexString(activationTime)+")") ;
        // write numberOfVCRecords
        dc.indent();dc.getPs().println("numberOfVCRecords="+numberOfVCRecords+"(0x"+ Integer.toHexString(numberOfVCRecords) + ")" );
        // write vcRecords
        { ArrayList<virtual_channel> temp1 = vcRecords;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     virtual_channel element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            vcRecords.add(val); return vcRecords.size();
    }
    
    
    public int removeFromVcRecords(virtual_channel val)
    {
            vcRecords.remove(val); return vcRecords.size();
    }
    
    
    public int removeNthFromVcRecords(int idx)
    {
            vcRecords.remove(idx); return vcRecords.size();
    }
    
    
    public int emptyVcRecords(int idx)
    {
            vcRecords.clear(); return vcRecords.size();
    }
