...
// member static classes
    public  static  class second extends BinStruct implements PublicBinMsg {
        ...
        public int read(DataInputStream istream) throws IOException
        {
            ...
            {
                tuneFrequencyValu=istream.readUnsignedShort();
                retVal+=2;
            }
            // read modulationvalue
            {
                modulationvalue=(short)(istream.readUnsignedByte());
                retVal+=1;
            }
        ...
        public int write(DataOutputStream ostream) throws IOException
        {
            ...
            ostream.writeShort(tuneFrequencyValu);
            retVal +=2;
            // write modulationvalue
            ostream.writeByte(modulationvalue);
            retVal +=1;
            postWrite();
            return retVal;
        }
        ...
        public int dump(DumpContext dc) throws IOException
        {
            ...
            // write tuneFrequencyValu
            dc.indent();
            dc.getPs().println("tuneFrequencyValu="+tuneFrequencyValu+"(0x"+ Integer.toHexString(tuneFrequencyValu) + ")" );
            // write modulationvalue
            dc.indent();
            dc.getPs().println("modulationvalue="+modulationvalue+"(0x"+ Integer.toHexString(modulationvalue) + ")" );
            dc.decreaseIndent();
            return retVal;
        }
...
// End of code
    ...
    public int readNoHeader(DataInputStream istream) throws IOException
    {
        ...
        // read tuneType
        {
            tuneType=(short)(istream.readUnsignedByte());
            retVal+=1;
        }
        // read firstInstance
        if(tuneType ==0x00) {
            firstInstance = new first(this);
            retVal += firstInstance.read(istream);
        }
        // read secondInstance
        if(tuneType ==0x01) {
            secondInstance = new second(this);
    ...
    public int dump(DumpContext dc) throws IOException
    {
        ...
        // write tuneType
        dc.indent();
        dc.getPs().println("tuneType="+tuneType+"(0x"+ Integer.toHexString(tuneType) + ")" );
        // write firstInstance
