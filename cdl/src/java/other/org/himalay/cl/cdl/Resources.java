...
public  class Resources extends BinStruct implements PublicBinMsg {
    ...
    private void init()
    {
        ...
        reourceList= new ArrayList<ResourceFactory.Resource>();
    ...
    public int read(DataInputStream istream) throws IOException
    {
        ...
        for (int iIdx=0; iIdx <descriptor_count + (0); iIdx++) {
            ResourceFactory.Resource temp;    /* Generic classes are abstract, so we can not invoke new*/    {
    ...
    public int write(DataOutputStream ostream) throws IOException
    {
        ...
        // write reourceList
        {
            ArrayList<ResourceFactory.Resource> temp1 = reourceList;
            for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++) {
                ResourceFactory.Resource temp2    =    temp1.get(iIdx);
    ...
    public int dump(DumpContext dc) throws IOException
    {
        ...
        // write descriptor_count
        dc.indent();
        dc.getPs().println("descriptor_count="+descriptor_count+"(0x"+ Integer.toHexString(descriptor_count) + ")" );
        // write reourceList
        {
            ArrayList<ResourceFactory.Resource> temp1 = reourceList;
            for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++) {
                ResourceFactory.Resource element    = temp1.get(iIdx);
                dc.indent();
