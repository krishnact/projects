...
public  class APDUFlowConf_Detail extends BinStruct implements PublicBinMsg{
    ...
    preRead();
        ...
        // read socket
        if (serviceType==0x04){ socket = new ServiceSocket();retVal += socket.read(istream);        }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("APDUFlowConf_Detail\n");
    dc.increaseIndent();
        int retVal= 0;
        // write flowId
        ...
        // write socket
        if ( socket != null ) {dc.indent();dc.getPs().println("socket") ;retVal +=socket.dump(dc);}
dc.decreaseIndent();
        return retVal;
    }
