...
public  class dvbJapp_locator_descriptor extends   app_desc_baseFactory.app_desc_base { //Concrete type is dvbJapp_locator_descriptor
    ...
    private void init()
    {
        ...
        base_directory= new ByteArray();base_directory.setSizeType("FIRST_UI8");
        // Initialize class_path
        class_path= new ByteArray();class_path.setSizeType("FIRST_UI8");
        // Initialize initial_class
        initial_class= new ByteArray();initial_class.setSizeType("EOS");
    }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("dvbJapp_locator_descriptor\n");
    dc.increaseIndent();
        int retVal= 0;
        // write desc_header
        if ( desc_header != null ) {dc.indent();dc.getPs().println("desc_header") ;retVal +=desc_header.dump(dc);}
        // write base_directory
        dc.indent();dc.getPs().print("base_directory: "+base_directory.getSize()+"(0x"+Integer.toHexString(base_directory.getSize())+")\n");this.base_directory.dump(dc);
        // write class_path
        dc.indent();dc.getPs().print("class_path: "+class_path.getSize()+"(0x"+Integer.toHexString(class_path.getSize())+")\n");this.class_path.dump(dc);
        // write initial_class
        dc.indent();dc.getPs().print("initial_class: "+initial_class.getSize()+"(0x"+Integer.toHexString(initial_class.getSize())+")\n");this.initial_class.dump(dc);
dc.decreaseIndent();
        return retVal;
    }
