...
public  class ECM_statusReport extends BinStruct implements PublicBinMsg {
    ...
    public int read(DataInputStream istream) throws IOException
    {
        ...
        {
            downstreamCenterFreq=istream.readUnsignedShort();
            retVal+=2;
        }
        // read downstreamPowerLevel
        {
            downstreamPowerLevel=istream.readUnsignedShort();
            retVal+=2;
        }
        // read bf1
        ...
        {
            upstreamXmtCenterFreq=istream.readUnsignedShort();
            retVal+=2;
        }
        // read upstreamPowerLevel
        {
            upstreamPowerLevel=istream.readUnsignedShort();
            retVal+=2;
        }
        // read upstreamSymbolRate
        {
            upstreamSymbolRate=(short)(istream.readUnsignedByte());
            retVal+=1;
        }
    ...
    public int write(DataOutputStream ostream) throws IOException
    {
        ...
        ostream.writeShort(downstreamCenterFreq);
        retVal +=2;
        // write downstreamPowerLevel
        ostream.writeShort(downstreamPowerLevel);
        retVal +=2;
        // write bf1
        ostream.writeByte(bf1.getValue());
        retVal +=1;
        // write upstreamXmtCenterFreq
        ostream.writeShort(upstreamXmtCenterFreq);
        retVal +=2;
        // write upstreamPowerLevel
        ostream.writeShort(upstreamPowerLevel);
        retVal +=2;
        // write upstreamSymbolRate
        ostream.writeByte(upstreamSymbolRate);
        retVal +=1;
        postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException
    {
        ...
        // write downstreamCenterFreq
        dc.indent();
        dc.getPs().println("downstreamCenterFreq="+downstreamCenterFreq+"(0x"+ Integer.toHexString(downstreamCenterFreq) + ")" );
        // write downstreamPowerLevel
        dc.indent();
        dc.getPs().println("downstreamPowerLevel="+downstreamPowerLevel+"(0x"+ Integer.toHexString(downstreamPowerLevel) + ")" );
        // write bf1
        {
            dc.indent();
            dc.getPs().print("downstreamCarrierLockStatus: ");
            dc.getPs().println(BitField_8.toDisplayString(getDownstreamCarrierLockStatus(),1));
            dc.indent();
            dc.getPs().print("reserved: ");
            dc.getPs().println(BitField_8.toDisplayString(getReserved(),2));
            dc.indent();
            dc.getPs().print("channel_s_cdmaStatus: ");
            dc.getPs().println(BitField_8.toDisplayString(getChannel_s_cdmaStatus(),2));
            dc.indent();
            dc.getPs().print("upstreamModulationType: ");
            dc.getPs().println(BitField_8.toDisplayString(getUpstreamModulationType(),3));
        }
        // write upstreamXmtCenterFreq
        dc.indent();
        dc.getPs().println("upstreamXmtCenterFreq="+upstreamXmtCenterFreq+"(0x"+ Integer.toHexString(upstreamXmtCenterFreq) + ")" );
        // write upstreamPowerLevel
        dc.indent();
        dc.getPs().println("upstreamPowerLevel="+upstreamPowerLevel+"(0x"+ Integer.toHexString(upstreamPowerLevel) + ")" );
        // write upstreamSymbolRate
        dc.indent();
        dc.getPs().println("upstreamSymbolRate="+upstreamSymbolRate+"(0x"+ Integer.toHexString(upstreamSymbolRate) + ")" );
        dc.decreaseIndent();
        return retVal;
    }
