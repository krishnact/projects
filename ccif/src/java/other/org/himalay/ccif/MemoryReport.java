...
public  class MemoryReport extends BinStruct implements PublicBinMsg{
    ...
    private void init()
    {
        ...
        numberOfMemory= new ByteArray();numberOfMemory.setSizeType("FIRST_UI8");
    }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("MemoryReport\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        if ( header != null ) {dc.indent();dc.getPs().println("header") ;retVal +=header.dump(dc);}
        // write numberOfMemory
        dc.indent();dc.getPs().print("numberOfMemory: "+numberOfMemory.getSize()+"(0x"+Integer.toHexString(numberOfMemory.getSize())+")\n");this.numberOfMemory.dump(dc);
dc.decreaseIndent();
        return retVal;
    }
