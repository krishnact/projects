...
public  class NewFlowConfirm extends   APDUBase_1Factory.APDUBase_1 { //Concrete type is NewFlowConfirm
  ...
  preRead();
    int retVal= 0;
        ...
        // read flowDetails
        if (statusField==0x00){ flowDetails = new APDUFlowConf_Detail();retVal += flowDetails.read(istream);        }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("NewFlowConfirm\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        ...
        // write flowDetails
        if ( flowDetails != null ) {dc.indent();dc.getPs().println("flowDetails") ;retVal +=flowDetails.dump(dc);}
dc.decreaseIndent();
        return retVal;
    }
