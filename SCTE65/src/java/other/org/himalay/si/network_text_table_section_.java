...
public  class network_text_table_section_ extends   SCTE65TableFactory.SCTE65Table { //Concrete type is network_text_table_section
...
// member static classes
    public  static  class source_name_subtable extends BinStruct implements PublicBinMsg{


// member static classes
    public  static  class SNSData extends BinStruct implements PublicBinMsg{
...
source_name_subtable parent;
    ...
    private void init()
    {
        ...
        sourceName= new ByteArray();sourceName.setSizeType("FIRST_UI8");
    ...
    preRead();
        ...
        {appIdOrSourceType=istream.readUnsignedShort(); retVal+=2;}
        // read sourceName
    ...
    preWrite();
        ...
        ostream.writeShort(appIdOrSourceType); retVal +=2;
        // write sourceName
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("SNSData\n");
    dc.increaseIndent();
        int retVal= 0;
        // write bf1
        {dc.indent();dc.getPs().print("applicationType: ");dc.getPs().println(BitField_8.toDisplayString(getApplicationType(),1));}
        // write appIdOrSourceType
        dc.indent();dc.getPs().println("appIdOrSourceType="+appIdOrSourceType+"(0x"+ Integer.toHexString(appIdOrSourceType) + ")" );
        // write sourceName
        dc.indent();dc.getPs().print("sourceName: "+sourceName.getSize()+"(0x"+Integer.toHexString(sourceName.getSize())+")\n");this.sourceName.dump(dc);
        // write descriptors
...
subtable parent;
    ...
    private void init()
    {
        ...
        // Initialize snsAray
        snsAray= new ArrayList<SNSData>();snsAray.setMemberSize(0);
    }
    ...
    preRead();
        int retVal= 0;
                     ...
                     // read count
        {count=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read snsAray
        for (int iIdx=0; iIdx <count + (0); iIdx++){     SNSData temp;    temp = new SNSData(this);    retVal += temp.read(istream);    snsAray.add(temp);}
    ...
    preWrite();
        ...
        { /** fix dependent sizes for snsAray**/ count=((short)snsAray.getCount()); }
    
                // write count
        ostream.writeByte(count); retVal +=1;
        // write snsAray
        {ArrayList<SNSData> temp1 = snsAray;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     SNSData temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("source_name_subtable\n");
    dc.increaseIndent();
        int retVal= 0;
        // write count
        dc.indent();dc.getPs().println("count="+count+"(0x"+ Integer.toHexString(count) + ")" );
        // write snsAray
        { ArrayList<SNSData> temp1 = snsAray;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     SNSData element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            snsAray.add(val); return snsAray.size();
    }
    
    
    public int removeFromSnsAray(SNSData val)
    {
            snsAray.remove(val); return snsAray.size();
    }
    
    
    public int removeNthFromSnsAray(int idx)
    {
            snsAray.remove(idx); return snsAray.size();
    }
    
    
    public int emptySnsAray(int idx)
    {
            snsAray.clear(); return snsAray.size();
    }
...
network_text_table_section_ parent;
    ...
    preRead();
        ...
        if ( parent.getTable_subtype()==6){ source_name_subtableInstance = new source_name_subtable(this);retVal += source_name_subtableInstance.read(istream);        }
        // read descriptor
    ...
    preWrite();
        ...
        if (source_name_subtableInstance!=null)retVal +=source_name_subtableInstance.write(ostream);
        // write descriptor
...
postWrite();
    ...
    dc.increaseIndent();
        int retVal= 0;
        // write source_name_subtableInstance
        if ( source_name_subtableInstance != null ) {dc.indent();dc.getPs().println("source_name_subtableInstance") ;retVal +=source_name_subtableInstance.dump(dc);}
        // write descriptor
...
// End of code
    ...
    private void init()
    {
        ...
        languageCode= new ByteArray();languageCode.setSizeType("FIXED");languageCode.setSize(3);
        // Initialize bf3
        bf3 = new BitField_8();
  ...
  preRead();
    int retVal= 0;
        ...
        // read subtableInstance
            {        int iCount    = tableHeader.getSection_length()+(-9);        byte[] ba        = new byte[iCount];istream.readFully(ba);ByteArrayInputStream baTemp= new ByteArrayInputStream(ba);        retVal+= subtableInstance.read(new DataInputStream(baTemp));}
        // read CRC
...
postWrite();
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("network_text_table_section_\n");
    dc.increaseIndent();
        int retVal= 0;
        // write tableHeader
        ...
        // write languageCode
        dc.indent();dc.getPs().print("languageCode: "+languageCode.getSize()+"(0x"+Integer.toHexString(languageCode.getSize())+")\n");this.languageCode.dump(dc);
        // write bf3
