...
public  class code_version_table2_ extends   code_version_tableFactory.code_version_table { //Concrete type is code_version_table2
    ...
    private void init()
    {
        ...
        cvCertificates= new ArrayList<CVCertificate>();cvCertificates.setMemberSize(0);
    }
  ...
  preRead();
    int retVal= 0;
        ...
        {configuration_count_change=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read resources
        retVal += resources.read(istream);
        // read pvData
        {IntegerHolder iHolder= new IntegerHolder();DataInputStream disTemp = istream;pvData=PVDataFactory.createMsg(protocol_version,disTemp,iHolder); retVal+= iHolder.getValue();        }
        // read certificateCount
        {certificateCount=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read cvCertificates
        for (int iIdx=0; iIdx <certificateCount + (0); iIdx++){     CVCertificate temp;    temp = new CVCertificate();    retVal += temp.read(istream);    cvCertificates.add(temp);}
...
postRead();
    ...
    preWrite();
        ...
        ostream.writeByte(configuration_count_change); retVal +=1;
        // write resources
        if (resources!=null)retVal +=resources.write(ostream);
        // write pvData
        if (pvData!=null)retVal +=pvData.write(ostream);
        // write certificateCount
        ostream.writeByte(certificateCount); retVal +=1;
        // write cvCertificates
        {ArrayList<CVCertificate> temp1 = cvCertificates;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     CVCertificate temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("code_version_table2_\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        ...
        // write configuration_count_change
        dc.indent();dc.getPs().println("configuration_count_change="+configuration_count_change+"(0x"+ Integer.toHexString(configuration_count_change) + ")" );
        // write resources
        if ( resources != null ) {dc.indent();dc.getPs().println("resources") ;retVal +=resources.dump(dc);}
        // write pvData
        if ( pvData != null ) {dc.indent();dc.getPs().println("pvData") ;retVal +=pvData.dump(dc);}
        // write certificateCount
        dc.indent();dc.getPs().println("certificateCount="+certificateCount+"(0x"+ Integer.toHexString(certificateCount) + ")" );
        // write cvCertificates
        { ArrayList<CVCertificate> temp1 = cvCertificates;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     CVCertificate element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            cvCertificates.add(val); return cvCertificates.size();
    }
    
    
    public int removeFromCvCertificates(CVCertificate val)
    {
            cvCertificates.remove(val); return cvCertificates.size();
    }
    
    
    public int removeNthFromCvCertificates(int idx)
    {
            cvCertificates.remove(idx); return cvCertificates.size();
    }
    
    
    public int emptyCvCertificates(int idx)
    {
            cvCertificates.clear(); return cvCertificates.size();
    }
