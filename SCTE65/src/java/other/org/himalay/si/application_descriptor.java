...
public class application_descriptor extends app_desc_baseFactory.app_desc_base { // Concrete
																					// type
																					// is
	...
	public static class profile extends BinStruct implements PublicBinMsg {
		...
		public int read(DataInputStream istream) throws IOException {
			...
			// read app_profile
			{
				app_profile = istream.readUnsignedShort();
				retVal += 2;
			}
			// read major_version
			{
				major_version = (short) (istream.readUnsignedByte());
				retVal += 1;
			}
			// read minor_version
			{
				minor_version = (short) (istream.readUnsignedByte());
				retVal += 1;
			}
			// read micro_version
			{
				micro_version = (short) (istream.readUnsignedByte());
				retVal += 1;
			}
		...
		public int write(DataOutputStream ostream) throws IOException {
			...
			// write app_profile
			ostream.writeShort(app_profile);
			retVal += 2;
			// write major_version
			ostream.writeByte(major_version);
			retVal += 1;
			// write minor_version
			ostream.writeByte(minor_version);
			retVal += 1;
			// write micro_version
			ostream.writeByte(micro_version);
			retVal += 1;
			postWrite();
			return retVal;
		}
		...
		public int dump(DumpContext dc) throws IOException {
			...
			dc.indent();
			dc.getPs().println(
					"app_profile=" + app_profile + "(0x"
							+ Integer.toHexString(app_profile) + ")");
			// write major_version
			dc.indent();
			dc.getPs().println(
					"major_version=" + major_version + "(0x"
							+ Integer.toHexString(major_version) + ")");
			// write minor_version
			dc.indent();
			dc.getPs().println(
					"minor_version=" + minor_version + "(0x"
							+ Integer.toHexString(minor_version) + ")");
			// write micro_version
			dc.indent();
			dc.getPs().println(
					"micro_version=" + micro_version + "(0x"
							+ Integer.toHexString(micro_version) + ")");
			dc.decreaseIndent();
			return retVal;
		}
	...
	public int readNoHeader(DataInputStream istream) throws IOException {
		...
		if (sizeLimit > 0) {
			...
			istream = new DataInputStream(baTemp);
			{
				...
				{
					app_profile_length = (short) (istream.readUnsignedByte());
					retVal += 1;
				}
				// read profiles
				{
				...
				{
					service_bound_visibility = (short) (istream
							.readUnsignedByte());
					retVal += 1;
				}
				// read priority
				{
					priority = (short) (istream.readUnsignedByte());
					retVal += 1;
				}
				// read transport_label
				{
					transport_label = (short) (istream.readUnsignedByte());
					retVal += 1;
				}
	...
	public int write(DataOutputStream ostream) throws IOException {
		...
		{
			/** fix dependent sizes for profiles **/
			app_profile_length = ((short) profiles.getSize());
		}
		...
		ostream.writeByte(app_profile_length);
		retVal += 1;
		// write profiles
		{
			ArrayList<profile> temp1 = profiles;
			for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
				profile temp2 = temp1.get(iIdx);
				if (temp2 != null)
					retVal += temp2.write(ostream);
			}
		}
		// write service_bound_visibility
		ostream.writeByte(service_bound_visibility);
		retVal += 1;
		// write priority
		ostream.writeByte(priority);
		retVal += 1;
		// write transport_label
		ostream.writeByte(transport_label);
		retVal += 1;
		postWrite();
		return retVal;
	}
	...
	public int dump(DumpContext dc) throws IOException {
		...
		dc.indent();
		dc.getPs().println(
				"app_profile_length=" + app_profile_length + "(0x"
						+ Integer.toHexString(app_profile_length) + ")");
		// write profiles
		{
			ArrayList<profile> temp1 = profiles;
			for (int iIdx = 0; iIdx < temp1.getCount(); iIdx++) {
				profile element = temp1.get(iIdx);
				dc.indent();
		...
		dc.indent();
		dc.getPs().println(
				"service_bound_visibility=" + service_bound_visibility + "(0x"
						+ Integer.toHexString(service_bound_visibility) + ")");
		// write priority
		dc.indent();
		dc.getPs().println(
				"priority=" + priority + "(0x" + Integer.toHexString(priority)
						+ ")");
		// write transport_label
		dc.indent();
		dc.getPs().println(
				"transport_label=" + transport_label + "(0x"
						+ Integer.toHexString(transport_label) + ")");
		dc.decreaseIndent();
		return retVal;
	}
