...
public  class diagnosticReq_1_Mmode extends   APDUBaseFactory.APDUBase { //Concrete type is diagnosticReq_1_Mmode
    ...
    private void init()
    {
        ...
        selfDiagnostics= new ArrayList<diagnosticReq1mMosemsg>();selfDiagnostics.setMemberSize(0);
    }
  ...
  preRead();
    int retVal= 0;
        ...
        for (; istream.available() > 0 ; ){     diagnosticReq1mMosemsg temp;    temp = new diagnosticReq1mMosemsg();    retVal += temp.read(istream);    selfDiagnostics.add(temp);}
...
postRead();
    ...
    preWrite();
        ...
        {ArrayList<diagnosticReq1mMosemsg> temp1 = selfDiagnostics;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     diagnosticReq1mMosemsg temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("diagnosticReq_1_Mmode\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        ...
        // write selfDiagnostics
        { ArrayList<diagnosticReq1mMosemsg> temp1 = selfDiagnostics;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     diagnosticReq1mMosemsg element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            selfDiagnostics.add(val); return selfDiagnostics.size();
    }
    
    
    public int removeFromSelfDiagnostics(diagnosticReq1mMosemsg val)
    {
            selfDiagnostics.remove(val); return selfDiagnostics.size();
    }
    
    
    public int removeNthFromSelfDiagnostics(int idx)
    {
            selfDiagnostics.remove(idx); return selfDiagnostics.size();
    }
    
    
    public int emptySelfDiagnostics(int idx)
    {
            selfDiagnostics.clear(); return selfDiagnostics.size();
    }
