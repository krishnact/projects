...
public  class ICM_structure_base extends   S_VCT_SubtableFactory.S_VCT_Subtable { //Concrete type is ICM_structure
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("icmData\n");
    dc.increaseIndent();
        int retVal= 0;
        // write sourceId
        dc.indent();dc.getPs().println("sourceId="+sourceId+"(0x"+ Integer.toHexString(sourceId) + ")" );
        // write bf5
        {dc.indent();dc.getPs().print("zero: ");dc.getPs().println(BitField_8.toDisplayString(getZero(),4));dc.indent();dc.getPs().print("virtual_channel_number: ");dc.getPs().println(BitField_8.toDisplayString(getVirtual_channel_number(),12));}
dc.decreaseIndent();
        return retVal;
    }
...
// End of code
    ...
    private void init()
    {
        ...
        bf2 = new BitField_8();
        // Initialize icmDatum
        icmDatum= new ArrayList<icmData>();icmDatum.setMemberSize(0);
    }
    ...
    preRead();
        ...
        // read icmDatum
        for (int iIdx=0; iIdx <getRecord_count() + (0); iIdx++){     icmData temp;    temp = new icmData(this);    retVal += temp.read(istream);    icmDatum.add(temp);}
    ...
    preWrite();
        ...
        { /** fix dependent sizes for icmDatum**/ setRecord_count((short)icmDatum.getCount()); }
        ...
        // write icmDatum
        {ArrayList<icmData> temp1 = icmDatum;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     icmData temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("ICM_structure_base\n");
    dc.increaseIndent();
        int retVal= 0;
        // write bf1
        {dc.indent();dc.getPs().print("zero: ");dc.getPs().println(BitField_8.toDisplayString(getZero(),4));dc.indent();dc.getPs().print("first_map_index: ");dc.getPs().println(BitField_8.toDisplayString(getFirst_map_index(),12));}
        // write bf2
        {dc.indent();dc.getPs().print("zero2: ");dc.getPs().println(BitField_8.toDisplayString(getZero2(),1));dc.indent();dc.getPs().print("record_count: ");dc.getPs().println(BitField_8.toDisplayString(getRecord_count(),7));}
        // write icmDatum
        { ArrayList<icmData> temp1 = icmDatum;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     icmData element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            icmDatum.add(val); return icmDatum.size();
    }
    
    
    public int removeFromIcmDatum(icmData val)
    {
            icmDatum.remove(val); return icmDatum.size();
    }
    
    
    public int removeNthFromIcmDatum(int idx)
    {
            icmDatum.remove(idx); return icmDatum.size();
    }
    
    
    public int emptyIcmDatum(int idx)
    {
            icmDatum.clear(); return icmDatum.size();
    }
