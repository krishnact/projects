...
PV2 parent;
    ...
    private void init()
    {
        ...
        object_data= new ByteArray();object_data.setSizeType("FIRST_UI8");
        // Initialize code_file_name
        code_file_name= new ByteArray();code_file_name.setSizeType("FIRST_UI8");
    }
    ...
    preRead();
        ...
        // read downloadType_0
        if (getDownload_type() == 0){ downloadType_0 = new DownloadType_0_or_1();retVal += downloadType_0.read(istream);        }
        // read downloadType_1
        if (getDownload_type() == 0){ downloadType_1 = new DownloadType_0_or_1();retVal += downloadType_1.read(istream);        }
        // read downloadType_2
        if (getDownload_type() == 2){ downloadType_2 = new DownloadType_2();retVal += downloadType_2.read(istream);        }
        // read object_type
        {object_type=istream.readUnsignedShort(); retVal+=2;}
        // read object_data
    ...
    preWrite();
        ...
        // write object_type
        ostream.writeShort(object_type); retVal +=2;
        // write object_data
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("PV2Object\n");
    dc.increaseIndent();
        int retVal= 0;
        // write bf1
        {dc.indent();dc.getPs().print("download_type: ");dc.getPs().println(BitField_8.toDisplayString(getDownload_type(),4));dc.indent();dc.getPs().print("download_command: ");dc.getPs().println(BitField_8.toDisplayString(getDownload_command(),4));}
        // write downloadType_0
        if ( downloadType_0 != null ) {dc.indent();dc.getPs().println("downloadType_0") ;retVal +=downloadType_0.dump(dc);}
        // write downloadType_1
        if ( downloadType_1 != null ) {dc.indent();dc.getPs().println("downloadType_1") ;retVal +=downloadType_1.dump(dc);}
        // write downloadType_2
        if ( downloadType_2 != null ) {dc.indent();dc.getPs().println("downloadType_2") ;retVal +=downloadType_2.dump(dc);}
        // write object_type
        dc.indent();dc.getPs().println("object_type="+object_type+"(0x"+ Integer.toHexString(object_type) + ")" );
        // write object_data
        dc.indent();dc.getPs().print("object_data: "+object_data.getSize()+"(0x"+Integer.toHexString(object_data.getSize())+")\n");this.object_data.dump(dc);
        // write code_file_name
        dc.indent();dc.getPs().print("code_file_name: "+code_file_name.getSize()+"(0x"+Integer.toHexString(code_file_name.getSize())+")\n");this.code_file_name.dump(dc);
dc.decreaseIndent();
        return retVal;
    }
...
// End of code
    ...
    private void init()
    {
        ...
        // Initialize pv2Datum
        pv2Datum= new ArrayList<PV2Object>();pv2Datum.setMemberSize(0);
    }
    ...
    preRead();
        ...
        {number_of_objects=(short)(istream.readUnsignedByte()); retVal+=1;}
        // read pv2Datum
        for (int iIdx=0; iIdx <number_of_objects + (0); iIdx++){     PV2Object temp;    temp = new PV2Object(this);    retVal += temp.read(istream);    pv2Datum.add(temp);}
    ...
    preWrite();
        ...
        ostream.writeByte(number_of_objects); retVal +=1;
        // write pv2Datum
        {ArrayList<PV2Object> temp1 = pv2Datum;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     PV2Object temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    public int dump(DumpContext dc) throws IOException 
    {
        dc.indent();dc.getPs().print("PV2\n");
    dc.increaseIndent();
        int retVal= 0;
        // write number_of_objects
        dc.indent();dc.getPs().println("number_of_objects="+number_of_objects+"(0x"+ Integer.toHexString(number_of_objects) + ")" );
        // write pv2Datum
        { ArrayList<PV2Object> temp1 = pv2Datum;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     PV2Object element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
    ...
    {
            pv2Datum.add(val); return pv2Datum.size();
    }
    
    
    public int removeFromPv2Datum(PV2Object val)
    {
            pv2Datum.remove(val); return pv2Datum.size();
    }
    
    
    public int removeNthFromPv2Datum(int idx)
    {
            pv2Datum.remove(idx); return pv2Datum.size();
    }
    
    
    public int emptyPv2Datum(int idx)
    {
            pv2Datum.clear(); return pv2Datum.size();
    }
