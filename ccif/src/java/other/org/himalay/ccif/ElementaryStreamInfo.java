...
public  class ElementaryStreamInfo extends BinStruct implements PublicBinMsg{
    ...
    preRead();
        ...
        // read caInfo
            {        int iCount    = getElementaryStreamInfoLength()+(0);        byte[] ba        = new byte[iCount];istream.readFully(ba);ByteArrayInputStream baTemp= new ByteArrayInputStream(ba);        retVal+= caInfo.read(new DataInputStream(baTemp));}
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("ElementaryStreamInfo\n");
    dc.increaseIndent();
        int retVal= 0;
        // write streamType
        dc.indent();dc.getPs().println("streamType="+streamType+"(0x"+ Integer.toHexString(streamType) + ")" );
        // write bf1
        {dc.indent();dc.getPs().print("reserved2: ");dc.getPs().println(BitField_8.toDisplayString(getReserved2(),3));dc.indent();dc.getPs().print("elementaryStreamInfoLength: ");dc.getPs().println(BitField_8.toDisplayString(getElementaryStreamInfoLength(),13));}
        // write caInfo
        if ( caInfo != null ) {dc.indent();dc.getPs().println("caInfo") ;retVal +=caInfo.dump(dc);}
dc.decreaseIndent();
        return retVal;
    }
