...
public  class ServiceIPMulticast extends BinStruct implements PublicBinMsg{
    ...
    private void init()
    {
        ...
        multicastGroupId= new ByteArray();multicastGroupId.setSizeType("FIXED");multicastGroupId.setSize(4);
    }
...
postWrite();
    ...
    {
        dc.indent();dc.getPs().print("ServiceIPMulticast\n");
    dc.increaseIndent();
        int retVal= 0;
        // write multicastGroupId
        dc.indent();dc.getPs().print("multicastGroupId: "+multicastGroupId.getSize()+"(0x"+Integer.toHexString(multicastGroupId.getSize())+")\n");this.multicastGroupId.dump(dc);
dc.decreaseIndent();
        return retVal;
    }
