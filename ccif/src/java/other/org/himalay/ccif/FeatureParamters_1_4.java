...
public  class FeatureParamters_1_4 extends   APDUBase_1Factory.APDUBase_1 { //Concrete type is FeatureParamters_1_4
    ...
    private void init()
    {
        ...
        featureList= new ArrayList<flist>();featureList.setMemberSize(0);
    }
  ...
  preRead();
    int retVal= 0;
        ...
        // read featureList
        for (; istream.available() > 0 ; ){     flist temp;    temp = new flist();    retVal += temp.read(istream);    featureList.add(temp);}
...
postRead();
    ...
    preWrite();
        ...
        // write featureList
        {ArrayList<flist> temp1 = featureList;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     flist temp2    =    temp1.get(iIdx);    if (temp2!=null)retVal +=temp2.write(ostream);}}
postWrite();
        return retVal;
    }
    ...
    {
        dc.indent();dc.getPs().print("FeatureParamters_1_4\n");
    dc.increaseIndent();
        int retVal= 0;
        // write header
        ...
        // write featureList
        { ArrayList<flist> temp1 = featureList;for (int iIdx=0; iIdx < temp1.getCount() ; iIdx++){     flist element    = temp1.get(iIdx); dc.indent(); dc.getPs().println(iIdx);    if ( element != null ) {dc.indent();dc.getPs().println("element") ;retVal +=element.dump(dc);}}}
dc.decreaseIndent();
        return retVal;
    }
